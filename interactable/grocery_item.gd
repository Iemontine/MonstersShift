...
func show_grocery_ui():
	...
	grocery_ui.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_set_button_icons(grocery_ui)
	_set_hint_texture(grocery_ui)
	_connect_buttons(grocery_ui)
	
	canvas_layer.transform.origin = get_viewport().get_camera_2d().global_position
	get_parent().get_node("CanvasModulate").color = Color(0.7, 0.7, 0.7)
	
	var prompt: Control = grocery_ui.get_node("Prompt")
	prompt.mouse_filter = Control.MOUSE_FILTER_PASS
	prompt.get_node("Yes").mouse_filter = Control.MOUSE_FILTER_PASS
	prompt.get_node("No").mouse_filter = Control.MOUSE_FILTER_PASS
